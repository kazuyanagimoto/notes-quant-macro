...
# ╔═╡ 40df8a27-b85c-49f5-ab20-41c31b46d972
begin
	using Parameters
	using QuantEcon
...
# ╔═╡ cfbd75e0-6be0-11ec-26d6-51a13c117f51
md"""
# Life-Cycle Model
#### Kazuharu Yanagimoto
...
# ╔═╡ e4a6f7de-8dcd-4a6b-ab2f-0281e769597f
...
# ╔═╡ a3cba233-c541-4ab9-a0f5-91026dc22de2
...
function solve_single(states::States)
	...
	for t = T:-1:1, iₓ= 1:nₓ, iₑ = 1:nₑ
		...
		for iₓ′ = 1:nₓ
			...
			if c > 0
				utility = max(u(c, states) + β * expected, utility)
			end
...
function solve_threads(states::States)
...
let
	...
	for (i, t) ∈ enumerate([1, 4, 7, 10])
		...
		p = plot(states.x_grid, V[:, 1, t], 
			xlabel = "x", 
			label = L"e_1", 
			legend = :bottomright, 
			title = "t = $t")
		plot!(states.x_grid, V[:, 8, t], label = L"e_8")
		plot!(states.x_grid, V[:, 15, t], label = L"e_{15}")
		push!(ps, p)
	end
